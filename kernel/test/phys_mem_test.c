...
TEST(AllocBlocks) {
  void* first_block = alloc_blocks(5);
...
TEST(AllocTenFreeMiddleAndReallocMiddle) {
  void* first_block = alloc_blocks(10);
  void* fifth_block = first_block + (PHYS_BLOCK_SIZE * 4);
  free_blocks(fifth_block, 3);
  for (int i = 0; i < 3; i++) {
    EXPECT_FALSE("Alloc ten, free middle, realloc middle/1",
      is_alloced(free_blocks + (PHYS_BLOCK_SIZE * i)));
  }
  void* new_alloc = alloc_blocks(3);
  for (int i = 0; i < 3; i++) {
